...
function* run(request,response) {
  ...
  while (nextExists && currentPage < MAX_PAGE) { 
    if(request.query.website!=null && request.query.website!="" && request.query.website!=undefined && links[currentPage-1]!=undefined && arrayContains(links[currentPage-1],request.query.website)){
      yield nightmare.end(); 
      response.send({data:links,page_number:(currentPage),index:(foundIndex+1)});
    }
    else{
